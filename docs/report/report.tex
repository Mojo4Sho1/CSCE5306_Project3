...
Node 2 was the initial leader (term 1).
After stopping its container, Node 6 was the first to time out and sent \rpc{RequestVote}
to all remaining nodes.
The remaining followers (Nodes 1, 3, 4, 5) all granted their votes because their
\texttt{current\_term} was still 1; Node 6 collected a 5-vote majority and became
leader for term 2 in approximately 2--3 seconds.
Node 6 immediately began sending \rpc{AppendEntries} heartbeats to the survivors,
and all five nodes transitioned to follower state under the new leader.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{screenshots/tc1_tc1_leader_crash_and_re_election.png}
...
\paragraph{Observed behaviour.}
Node 5 was the leader; Node 1 (a follower) was stopped.
While Node 1 was down the leader continued sending \rpc{AppendEntries} heartbeats to the
remaining five nodes, reaching a majority without Node 1.
When Node 1 was restarted, it logged \texttt{Server listening\ldots} and immediately
began receiving \rpc{AppendEntries} from Node 5, catching up to the current log state.
No re-election occurred because the majority (five of six nodes) was always intact.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{screenshots/tc2_tc2_follower_crash_and_recovery.png}
...
\paragraph{Observed behaviour.}
Node 5 (the leader) was paused.
The five surviving nodes detected the missing heartbeat and Node 6 won the resulting
election, becoming leader for term 2.
When Node 5 was unpaused, it resumed its stale heartbeat loop (still in term 1) and
sent \rpc{AppendEntries} to surviving nodes.
Upon receiving an \rpc{AppendEntries} from the new leader (Node 6, term 2), Node 5
saw a higher term, stepped down to follower, and began tracking Node 6 as leader.
The cluster stabilised with Node 6 as the sole leader for term 2.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{screenshots/tc3_tc3_network_partition_pause_unpause.png}
...
\paragraph{Observed behaviour.}
Node 4 was elected leader of the initial five-node cluster (term 1).
After starting \texttt{fishing6} with \texttt{docker compose up -d fishing6},
the new node logged \texttt{Server listening\ldots} and within the next heartbeat
interval (at most 1 second) received its first \rpc{AppendEntries} from Node 4.
The leader's log shows \texttt{Node 4 sends RPC AppendEntries to Node 6} added to
its heartbeat round immediately, confirming that Node 6 was included in the cluster
without any manual configuration step.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{screenshots/tc4_tc4_new_node_joining_the_cluster.png}
...
\paragraph{Observed behaviour.}
Node 2 (leader, term 1) and Node 3 (follower) were stopped simultaneously, leaving
four nodes (1, 4, 5, 6).
A majority of the 6-node cluster requires 4 votes.
\textbf{Term 2 --- split vote}: Nodes 5 and 6 both timed out and started elections for
term 2 at nearly the same moment.  Each voted for itself and sent \rpc{RequestVote}
to the others; the four available votes were split roughly evenly between the two
candidates and neither reached the 4-vote threshold.
\textbf{Term 3 --- retry and resolution}: Node 5 re-randomised its election timeout and
started a second election.  Logs show Node 5 sending \rpc{RequestVote} a second time
(\texttt{Node 5 sends RPC RequestVote to Node 1/2/3/4/6} repeated twice total) and
collecting a majority; Node 5 became leader for term 3.
The logs show Nodes 1 and 4 receiving \rpc{RequestVote} from both Node 6 (term 2) and
Node 5 (terms 2 and 3), confirming that multiple election rounds occurred before a
leader was established.

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{screenshots/tc5_tc5_split_vote_and_election_retry.png}
...
\begin{itemize}
  ...
  \item The \texttt{PEERS} environment variable must be set for \emph{every} node, not
    only the coordinator.  An initial misconfiguration left \texttt{PEERS=""} for nodes
    2--6; each node then treated itself as a single-node cluster and ``won'' every
    election with one self-vote.  The fix sets the full peer list for all six services
    in \texttt{docker-compose.yml}.
\end{itemize}
