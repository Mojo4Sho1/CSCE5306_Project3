...
1. **Run the Q5 notebook:** `jupyter notebook docs/report/q5_failure_tests.ipynb`
   - The notebook (`docs/report/q5_failure_tests.ipynb`) automates TC1–TC5 end-to-end:
     resets the cluster, runs each failure action, captures log output, and saves
     terminal-styled PNGs to `docs/report/screenshots/tc{1-5}_*.png`.
   - Run each TC section in order (top to bottom). Read the markdown cell before each code section.
   - If a TC fails unexpectedly, check `docs/spec/06_failure_test_matrix.md` for the manual Docker commands.
2. **Verify screenshots exist** (`ls docs/report/screenshots/`) — 5 PNGs expected.
3. **Fill in observed behaviour** — add a sentence or two per TC in `docs/report/report.tex` at each `\todo{Fill in after running the test…}` placeholder.
4. **Build the PDF** — `make pdf` (requires `pdflatex`; install via MacTeX or `texlive-latex-base`).
5. **Mandatory final subtask** — update handoff docs (see below).
...
- 5 test cases executed via `docs/report/q5_failure_tests.ipynb`
- 5 PNG screenshots in `docs/report/screenshots/` (generated by the notebook)
- `\todo{}` placeholders in `docs/report/report.tex` filled in with observed behaviour
- `make pdf` succeeds (PDF builds without errors)
